"""Generate PMF and CDF plots for Solution 01 (Discrete PMF given as a table)."""
...
xs = np.array([-2, 0, 1, 3, 5], dtype=float)
ps = np.array([0.10, 0.25, 0.30, 0.20, 0.15], dtype=float)
...
prev = np.concatenate(([0.0], cdf_vals[:-1]))
ax.plot(xs, prev, "o", mfc="white", mec="C3", markersize=7, label="open from below")
